...
export async function POST(req: NextRequest) {
    ...
    if (hasAccess) {
        ...
        const postObj: IPostSchema | null = await post.findByIdAndUpdate(postId, { $addToSet: { likes: userId } })
        const userdetails: IUserDetailsSchema|null=await userDetails.findOneAndUpdate({userId}, {$addToSet: {likes: postObj?._id}})

        if (postObj && userdetails) {
            const user=await users.findById(userId)
            const notification=await createNotification({about: "liked your post", link: postObj._id as string, recipentArr: [postObj.userId as unknown as string], senderName: user.firstName+" "+user.lastName})
